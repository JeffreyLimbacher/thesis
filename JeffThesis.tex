...
\abstract{Gas flows around high-speed high altitude aircraft and within rocket thrusters contains regions where the particle velocities deviates significantly from the Maxwellian distribution. The gas in these regions is said to be in non-continuum and is best described by kinetic equations. The most physically accurate model is given by the Boltzmann equation. Due to its high computational cost, there is great interest in reducing the computational costs associated with the evaluation of the collision integral. This thesis focuses on reducing the computational costs associated with the collision integral. The Discontinuous Galerkin (DG) method is used to discretize the collision integral. The DG method is advantageous for its ability to capture arbitrary surfaces and conserve mass, momentum, and temperature. The DG discretization of the Boltzmann equation is introduced. Discretization leads to a weighted convolution form which costs $\mathcal{O}(N^8)$ operations to calculate directly. A discussion of convolutions and their properties is also introduced. A method is introduced based on the Discrete Fourier Transform (DFT) to reduce the calculation of the convolution form down to $\mathcal{O}(N^6)$. Empirical results are shown. A brief discussion of a new approach based on model kinetic equations is also given.}

%\copyrightyear{2015}

\acknowledgement{My deepest gratitude goes to my advisor, Dr. Alekseenko, for his support throughout my master's program. I cherish the experience of working with Dr. Alekseenko. He has given given me amazing opportunities. Under his guidance, I have fulfilled a goal of completing an end-to-end research project. He has always been supportive of my decisions. There is no other person whose advice weighs more heavily in my mind than Dr. Alekseenko's. I would like to thank the CSUN Math department, which has a superb focus on its students. 

I would like to thank my family for their support throughout my time at CSUN. They have supported my decision to pursue a Master's degree without hesitation. Lastly, thanks to my girlfriend, Pauline, who has been a constant source of happiness for me.}
