...
\abstract{Gas flows around high-speed high altitude aircraft and within rocket thrusters contain regions where the particle velocities deviates significantly from the Maxwellian distribution. The gas in these regions is said to be in non-continuum and is best described by kinetic equations. The most physically accurate model is given by the Boltzmann equation. Due to its high computational cost, there is great interest in reducing the computational costs associated with the evaluation of the collision integral. This thesis focuses on reducing the computational costs associated with the collision integral. The Discontinuous Galerkin (DG) method is used to discretize the equation. Discretization leads to a weighted convolution form which costs $\mathcal{O}(N^8)$ operations to calculate directly. A discussion of convolutions, their properties, and fast evaluation of convolutions is discussed. A method is introduced based on the Discrete Fourier Transform (DFT) to reduce the calculation of the convolution form down to $\mathcal{O}(N^6)$. Empirical results are shown demonstrating that it is conservative and error is minimal. A discussion of two different formulations of the collision integrals are discussed and numerical results are given. A brief discussion of a new approach based on model kinetic equations is also given.}

%\copyrightyear{2015}

\acknowledgement{My deepest gratitude goes to my advisor, Dr. Alekseenko. Working with him has been an amazing experience. Dr. Alekseenko has given me amazing opportunities, guidance, and support. I would like to thank the CSUN Math department, which has a superb focus on its students. I would like to thank my friends and family for their support throughout my time at CSUN. They have supported my decision to pursue a Master's degree without hesitation. Lastly, thanks to my girlfriend, Pauline, who has been a constant source of happiness and motivation for me.}
...
Kinetic theory describes the non-equilibrium dynamics of a gas or any system comprised of a large number of particles. Kinetic equations have found applications in wide range of problems such as rarefied gas dynamics \cite{Kremer2010} \cite{Kogan1969}, radiative transfer, and semiconductors modeling. 
...
At the same time, direct numerical computation of the Boltzmann equation remains elusive.
The Boltzmann equation is composed of a five-fold integral which must be evaluated at all points in time, space, and velocity resulting in $\mathcal{O}(n^{12})$ computational cost where $n$ is the number of discretization points in space and velocity in each of the three dimensions. 
This thesis primarily concerns with evaluating the collision integral in velocity space which has of $\mathcal{O}(n^9)$ operations. This is still computationally prohibitive, there is a strong interest to develop methods that lower the costs.
...
In addition, another method is introduced which builds upon the ES-BGK method of Holway \cite{H66}, which will be referred to as the Rel-ES method. The Rel-ES method is fater than solving full Boltzmann equation. However, it is an approximation to the Boltzmann equation. We use solutions to the Boltzmann equations to evaluate the accuracy of the Rel-ES method.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% CHAPTER TWO
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\label{Chap2}
	...
	The kinetic theory of gases treats gases as composed of a large number of individual molecules that, for large periods of time, flow unimpeded. As these particles move freely through space, they collide with each other. Collisions of these particles drives the evolution of the gas towards equilibrium.
	
	We consider a gas enclosed in a volume. A single molecule of this gas can be described with having position $\vec{x}$ and velocity $\vec{v}$ at a time $t$. For a particular time, we can describe a molecule as a single point, $(\vec{x},\vec{v})$, in 6-dmensional space known as phase space. We describe the gas using a velocity distribution function, denoted by $f(t,\vecx,\vecv)$, such that $f(t,\vec{x},\vec{v})d\vec{x}\,d\vec{v}$ gives the number of particles whose position is within the range of $\vec{x} + d\vec{x}$ and whose velocities are within $\vec{v} + d\vec{v}$. 

In 1872 Boltzmann \cite{Boltzmann1872} introduced an equation which describes the time evolution of the distribution function $f$. In the absence of external forces and collisions of particles, the Boltzmann equation takes the form
\begin{equation}
...
\end{equation}
$I[f]$ is referred to as the collision operator. The explicit form of $I[f]$ depends on the properties of the gas. To describe it explicitly, we assume that the molecules of the gas composed entirely of a single species. In the next section, we derive the collision operator when particles are assumed to have hard sphere potentials. However, later on the thesis, a more general form of the collision operator is assumed. The extension to the general form from hard spheres is intuitive.
	
\section{Binary Collisions of Particles}
\label{sec:bincol}
	This section considers the properties of two particles on a collision path with each other as illustrated in Figure~\ref{fig:binary_collision}. In all the work that follows, it is assumed that the molecules undergo elastic hard sphere collisions. 
\begin{figure}[h]
	\centering
...
\end{equation*}
$b$ denotes the offset of the centers of the molecules orthogonal to $\vec{g}$. $\varepsilon$ denotes the azimuthal angle of the collision plane. 
...
\end{equation}
(\ref{eq:consv_momentum}) yields $|g'| = |g|$. In addition, due to the hard sphere assumption, the collision is considered to be perfectly elastic, giving
\begin{equation}
\label{eq:consv_kin}
...
In order to explicitly write the collision operator, we must describe the collisions within the gas. Consider a particle with velocity $\vecv_1$ at point $\vecx$. This particle will collide with $f(t,\vecx, \vecv)d\vecv_1 g \Delta t b \, db \, d\varepsilon$ particles within the ranges of $\vecv_1$ and $\vecv_1 + d\vecv_1$. Integrating this by  all possible azimuthal angles $(0 \leq \varepsilon < 2 \pi)$, over all impact parameters, $(0 \leq b \leq b^*)$, and over all velocities, for all particles within the volume element $\vecx d\vecx$, we observe that there are 
\begin{equation}
\label{eq:depletion}
...
If the gas is free from external influence, then the gas will approach an equilibrium. 
In this equilibrium, the gas distribution takes a specific shape known as the Maxwellian distribution is given below.
\begin{equation}
\label{eq:maxwellian}
...
The Maxwellian graph takes a bell shape that drops off rapidly as we move away from $\vec{\bar{v}}$, i.e. $\lim_{\lvert \vec{v} \lvert \rightarrow \infty} f_M(\vec{v},n, \vec{\bar{v}}, T) = 0 $. This will become important later when we discretize $f_M$ (see Section \ref{sec:periodic_cont}).
\begin{figure}[h]
\centering
...
When the gas is at equilibrium, the difference between the number of particles that enter and leave a particular phase volume vanishes. In other words, for the Maxwellian distribution,
\begin{equation*}
...
With these definitions, the relationship between the dimensionless macroparameters and the dimensional macroparameters can be obtained. We define
\begin{equation}
\begin{split}
...
We partition $K$ into $N$ smaller rectangular parallelpipeds $K_j = [u_L^j, u_R^j] \times [v_L^j, v_R^j] \times [w_L^j, w_R^j]$. Each $K_j$ will contain a set of basis function, $\phi_j^i$, $i=1,\dots, s$ as described. Next, we introduce nodes of Gauss quadratures of order $s_u$, $s_v$, and $s_w$ on each of the intervals $[u_L^j,u_R^j]$, $[v_L^j,v_R^j]$, and $[w_L^j,w_R^j]$. The nodes are denoted as
...
\end{equation}
We substitute (\ref{eq:3.4}) into (\ref{eq:boltzmann}), multiply the result by test basis function, integrate over $K_j$, and apply identity (\ref{eq:lemma2.1}) to arrive to
\begin{equation}
\label{discveloblzm}
...
kernel  $A(\vec{v},\vec{v}_{1};\phi_{i;j})$ will have only $\mathcal{O}(M^{5})$ non-zero components for each $\phi_{i;j}(\vec{v})$, 
where $M$ is the number of discrete velocity points in each velocity dimension. As a result, evaluation of  
(\ref{eq:transfer_eq}) will require $\mathcal{O}(M^{8})$ operations for each spatial point. This number of evaluations 
is very high. However, as we will show later, it can be reduced to $\mathcal{O}(M^6)$ operations using symmetries of
...
\begin{equation}
A(\vecv,\vecv_1;\phi_{i}^j) = A(\vecv_1,\vecv;\phi_{i}^j), \, \quad \forall \vecv, \vecv_1 \in \R^3. 
\end{equation}
Also,
\begin{equation}
A(\vecv,\vecv)=0, \quad \forall \vecv \in \R^3
\end{equation}
...
We note that in order to calculate (\ref{eq:I_dist}), it would require $\mathcal{O}(M^9)$ operations to calculate it, $\mathcal{O}(M^6)$ at one velocity node for $\mathcal{O}(M^3)$ velocity nodes. However, recall the property stated in Section~\ref{sec:reform_galerkin} that $A$ only has $\mathcal{O}(M^5)$ components bringing down the total cost of computing (\ref{eq:I_dist}) down to $\mathcal{O}(M^8)$ operations. However, we wish to reduce this further. It turns out that the Discrete Fourier Transform has properties that allow us to bring down the cost of computing (\ref{eq:I_dist}) from $\mathcal{O}(M^8)$ to $\mathcal{O}(M^6)$. The DFT and its properties are discussed in the next section.
...
\label{chap:dft}
The goal is to compute (\ref{eq:I_dist}) faster than $\mathcal{O}(M^8)$. We note that (\ref{eq:I_dist}) is similar to a discrete convolution form (which will be defined later). The Discrete Fourier Transform (DFT) can be used to speed up the computations of convolutions using the \textit{Convolution Theorem}. In order to apply the DFT to our problem, we must first assume a periodic extension of the discrete form of $f$. This section discusses the background of the Discrete Fourier Transform, convolution theorem, and periodic continuation needed to quickly compute (\ref{eq:I_dist}).
...
In calculations below we will use the following definition of the 
multidimensional DFT. Let 
$x_{k_1, \dots, k_d}$ be a sequence indexed by $k_1, \dots, k_d$ with equal 
lengths $N$ in each dimension. The DFT $\calF[x]_{k_1,\ldots,k_d}$ of 
$x_{n_1, \dots, n_d}$ is defined as 
\begin{equation}
\label{mdft_def}
\calF[x]_{k_1,\ldots,k_d} = \sum_{n_1=0}^{N-1} \left( W^{ k_1 n_1} 
\sum_{n_2=0}^{N-1} \left( W^{k_2 n_2} \dots 
\sum_{n_d=0}^{N-1} W^{k_d n_d } x_{n_1, \ldots, n_d}
\right) \right)\, .
\end{equation}
Also, it is useful to define the inverse of the transform, 
\begin{equation*}
%\label{mdft_inv}
x_{l_1,\ldots,l_d} = \frac{1}{N} \sum_{k_1=0}^{N-1} \left( W^{ -l_1 k_1} 
\frac{1}{N} \sum_{k_2=0}^{N-1} \left( W^{- l_2 k_{2}} \dots 
\frac{1}{N} \sum_{k_d=0}^{N-1} W^{-l_d k_d } \hat{x}_{k_1,\ldots,k_d} 
\right) \right)\, .
\end{equation*}
Similarly to one dimensional case, fast discrete Fourier transforms can be defined to 
evaluate the transform and its inverse in $\mathcal{O}(N^d\log N)$ operations. 
...
\calF^{-1}[\calF[x]]_l &= \frac{1}{N} \sum_{k=0}^{N-1}W^{-lk}(\sum_{j=0}^{N-1} W^{kj} x_j) \\
 ...
 &= N \frac{1}{N} x_l = x_l
\end{align*}
where the last step used that if $l \not = j$ then $\sum_{k=0}^{N-1} W^{(j-l)k} = 0$ and for $l=j$ we have that $\sum_{k=0}^{N-1} W^{(j-l)k} = \sum_{k=0}^{N-1} 1 = N$. Thus the definition given above returns the original sequence.
...
The convolution theorem seemingly gives no computational gain. Straightforward computation of the DFT and inverse DFT will take $\mathcal{O}(N^2)$ complex operations. Computation of (\ref{eq:cirlconv}) also takes $\mathcal{O}(N^2)$ operations without needing to perform any complex multiplications if both $\{x_n\}$ and $\{y_n\}$ are real. However, the complexity of calculating (\ref{eq:dft_1d}) can be reduced by an algorithm known as the Fast Fourier Transform (FFT) which can calculate both the forward and inverse DFT in $\mathcal{O}(N \log N)$ operations.
...
For ease of notation for later discussions, we introduce a linear convolution for infinite sequences. 
\begin{definition}
...
\calF[z]_k^* = \calF[x]_k \calF[y]_k \, . 
\end{equation}
Then we can expect that in general $\calF^{-1}[\calF[z]^*]_l \not = z_l$. We refer to this as \textit{aliasing error}. By trying to calculate the linear convolution using the covolution theorem, we are implicitly using a circular convolution in (\ref{eq:aliasing}). As discussed in Section~\ref{sec:lin_circ_eq}, taking an $L$-point DFT is insufficient to capture the correct values for $z_l$. 
...
\end{align}
We note that evaluating $\calF[\hat{I}]_{k_u}$ directly would require $\mathcal{O}(M^3)$ operations.
...
Lemma~\ref{lemma_dft} allows us to compute (\ref{FI_onedim_u}) in $\mathcal{O}(M^2)$ operations. Indeed, it takes 
$\mathcal{O}(M \log M)$ operations to compute $\calF^{-1}[f]_{k_{u}}$ using a fast Fourier transform and it takes 
$\mathcal{O}(M^2)$ operations to compute discrete convolution in the frequency space (\ref{onedfconv}). To extend this result to $\calF[I]_{k_{u},k_{v},k_{w}}$, it is sufficient to repeat the approach for indices $j_{v}$ and $j_{w}$ focusing on one dimension at a time. The following theorem summarizes the result. 
...
(\ref{eq:I_dist}) in $\mathcal{O}(s^9 M^6)$ operations using the algorithm outlined below. 
...
\begin{enumerate}
	\item The first step of the algorithm is to evaluate $\calF^{-1}[f_i]_{k_u,k_v,k_w}$. Evaluation of the inverse Fourier transform requires $\mathcal{O}(M^3 \log M)$ operations for each value of index $i$ by utilizing three-dimensional FFT. This must be repeated for each $i$, resulting in the total of $\mathcal{O}(s^9 M^3 \log M)$ operations where $s^3$ is the number of velocity nodes in each velocity cell.
    ...
    $\calF [I_{i,i',i''}]_{k_{u},k_{v},k_{w}}$ requires $\mathcal{O}(M^3)$ arithmetic operations. 
    There are $M^3$ combinations of $k_u,k_v,k_w$ and $s^9$ combinations of indices  $i$, $i'$, and $i''$, therefore complexity of this step is $\mathcal{O}(s^9 M^6)$.
...
= \sum_{i',i''=1}^s \calF[I_{i,i',i''}]_{k_{u},k_{v},k_{w}}\, . 
    \end{align*}
    This step requires adding $s^6$ sequences of length $M^3$ for every value of $i$, resulting in a complexity of $\mathcal{O}(s^9 M^3)$ operations. 
    \item We recover $\calF^{-1}[\calF[I_{i}]]_{j_u,j_v,j_w}=I_{i;j_u,j_v,j_w}'$.
    This requires calculating the three-dimensional inverse DFT for every $i$ which gives a complexity of $\mathcal{O}(s M^3 \log M)$.
\end{enumerate}

Overall, the algorithm has the numerical complexity of $\mathcal{O}(s^9 M^6)$ dominated by step 2. 
...
complexity $\mathcal{O}(M^6)$. In our simulations, we kept $s_u,s_v,s_w \leq 3$, however higher values 
...
\end{equation*}
where $j_{u;c}$ is canonical node, $j_u'$ is the old index in $I_{i;j_u,j_v,j_w}$, and the mod operation always returns the positive remainder. The final algorithm is given below.
...
However, this assumption does not make physical sense as it would imply there was infinite energy within the system.
...
Using this idea, we can still achieve good accuracy while using minimal zero-padding. We need only to pad by a small number of zeroes (generally $\leq 4$) to lose any effects of aliasing. This is shown in later numerical results.
...
The analysis in Section~\ref{sec:algorithm} suggested that the algorithm had a total number of operations of $\mathcal{O}(M^6)$ where $M$ is the number of velocity cells in one velocity dimension. 
As discussed in \cite{AlekseenkoJosyula2012a}, the direction evaluation of convolution (\ref{eq:I_dist}) requires $\mathcal{O}(M^8)$ operations. Thus we expect the FFT evaluation of (\ref{eq:I_dist}) to take much less time than the method discussed in \cite{AlekseenkoJosyula2012a}.
...
The computational complexity was modeled using $t=\mathcal{O}(M^\alpha)$ where $\alpha$ is a constant. 
...
$\mathcal{O}(M^8)$ arithmetic operations. 
...
In Figures~{\ref{fig02}} and Figure~{\ref{fig03}}, relaxation of moments in the 
...
costs, both in time and memory. Overall, however, the $\mathcal{O}(M^6)$ evaluation of 
...
Figure~\ref{fig:rel} shows the results of a spatially homogeneous relaxation using the Rel-ES method against a benchmark direct evaluation of the Boltzmann collision integral. Parameters $M=15$, $s=5$ were used to evaluate the shock wave. We can see that the Rel-ES method performs well in matching the benchmark solution. All of the moments closely follow that of the benchmark solution.
...
\chapter{Hierarchically Semi-Structured Compression of of the Kernel $A$}

The discrete form of the collision kernel $A(\vecv, \vecv_1;\phi_{i;j})$ defined by (\ref{eq:A_definition}) naturally is stored as a $N^3 \times N^3$ matrix. Although it is $N \times N$, many components are expected to be zero so that we only expect $\mathcal{O}(N^5)$ non-zero components of the discrete $A$ kernel. This property is exploited in \cite{AlekseenkoJosyula2012} to bring down the total number of operations to calculate (\ref{eq:I_dist}) down to $\mathcal{O}(N^8)$ operations. However, if the number of elements of $A$ can be reduced further, than it should lead to a computational speed up in the calculation of (\ref{eq:I_dist}).

Hierarchical Semi-Structured (HSS) matrix compression is a form of matrix compression that is appropriate for some dense forms of matrices. Under certain assumptions \cite{vandebril2010matrix, bebendorf2008hierarchical}, it is possible to compress matrices such that only $\mathcal{O}(N \log N)$ storage and $\mathcal{O}(N \log N)$ operations needed for matrix-vector multiplication. This would result in a significant improvement in speed and memory. HSS forms were successfully applied to matrices arising in integral equations of elasticity. It was found through empirical investigation that the kernel $A(\vecv,\vecv_1;\phi_{i;k})$ decreases to infinity in a manner similar to the elasticity potential.

HSS compression is complicated by the fact that both indices of the matrix represent points in three-dimensional space. For applying the compression, two techniques were used to enumerate the three dimensional rectangular velocity grid into a single index. In the first approach, we used an enumeration $j=m_vm_w (j_u-1)+m_w(j_v-1)+j_w$ where $(j_u,j_v,j_w)$ are the indices on a three-dimensional rectangular grid and $m_u$, $m_v$, and $m_w$ are the number of points in the $u$, $v$ and $w$ dimensions repsectively.
The other technique used Morton encoding. 

The sparisty structure of the operator $A(\vecv,\vecv_1;\phi_{i;j})$ in Figure~\ref{figX1}. It can be seen that the sparsity is very fractal i n the simple numbering and has much more structure in the Morton encoding. 
Morton encoding is a method to map multi-dimensional data to one dimension while preserving the locality of the data points. 

\begin{center}
\begin{figure}[h]
\centering
  \begin{tabular}{@{}cc@{}}
  \includegraphics[height=.215\textheight]{color.pdf}&
  \includegraphics[height=.215\textheight]{color_morton.pdf}
\end{tabular}
\caption{\label{figX1} Color plots of magnitudes of values of $A(\vec{v},\vec{v}_{1};\phi^{p}_{i,j_c})$ on uniform rectangle grid. (left) The case of simple numbering. (right) The case of Morton encoding.}
  \end{figure}
  \end{center}
  
HSS representation of the matrix  
$A(\vec{v},\vec{v}_{1};\phi^{p}_{i,j_c})$ was generated using the software package HLibPro (\texttt{www.hlibpro.com}).  
The results for $16\times 16\times 16$ grid are shown in Figure~\ref{figX2}. 
Each block in the figure represents a submatrix of the HSS matrix.
The colour of the block is green if the block is represented efficiently in a low 
rank format and is red if 
the block stored as a dense matrix. Curves visible inside the blocks represent logarithmic plots of the 
singular values of a submatrix. Numbers inside the blocks represents the number of singular values 
that are retained in low rank approximation. The cut off point for singular values is $\sigma<10^{-10}$. 
The white boxes are empty submatrices. In Figure~\ref{figX2}, the left plot corresponds to geometric clustering and 
the right plot corresponds to algebraic clustering of the matrix entries. The HSS decomposition was obtained from 
matrix $A(\vec{v},\vec{v}_{1};\phi^{p}_{i,j_c})$ in the Morton encoding. Lastly, in Figure~\ref{figX3} the case of 
$32\times 32\times 32$ is considered. The plot corresponds to the Morton encoding and algebraic clustering. 
\begin{center}
\begin{figure}[h]
\centering
\begin{tabular}{@{}cc@{}}
  \includegraphics[height=.255\textheight]{16x16_morton_stdgeom1.jpg}&
  \includegraphics[height=.255\textheight]{16x16_morton_weakgeom1.jpg}\\
\end{tabular}
\caption{\label{figX2} HSS reduction of a matrix form of $A(\vec{v},\vec{v}_{1};\phi^{p}_{i,j_c})$ in the Morton encoding. Case of $16\times 16 \times 16$ grid. (left) Geometrical partitioning. (right) Algebraic partitioning.}
\end{figure}
\end{center}

\begin{center}
\begin{figure}[h]
\includegraphics[height=.3\textheight]{32x32_nonmorton1.jpg}
\caption{\label{figX3} HSS reduction of a matrix form of $A(\vec{v},\vec{v}_{1};\phi^{p}_{i,j_c})$ in the Morton encoding. Case of $32\times 32 \times 32$ grid. Algebraic partitioning.}
\end{figure}
\end{center}

The HSS clustering appears to work as expected. It can be seen in the $32 \times 32 \times 32$ case that the diagonal achieved significant low rank approximate along the diagonal and lower half of the matrix. However, the achieved compression rates were not significant. One reason is that the matrix $A$ is already sparse. Typically HSS methods are  with $\mathcal{O}(N^2)$ entries whereas A is $\mathcal{O}(N^{5/3})$, therefore the reduction is insignificant. Another possible reason for the poor performance might be due to the fact that the matrix is six-dimensional and HSS algorithms based on asymptotic behaviors cannot be fully utilized. On the other hand, we applied to the matrix with only empirical evidence without appealing to the physical properties of the matrix. 
