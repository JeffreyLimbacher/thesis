...
\subsection{Linear Convolution as Circular Convolution Equivalence}
...
Suppose $\{x_n\}_{n=0}^{L-1}$ and $\{y_n\}_{n=0}^{P-1}$ For simplicity, let $L \geq P$. If we take an $L$-point DFT of $\{x_n\}$ or $\{y_n\}$ then we can perfectly reconstruct both $\{x_n\}$ and $\{y_n\}$ given $\{ \calF[x]_k \}$ and $\{ \calF[y]_k \}$. Now suppose $z_l$ is the linear convolution of $\{x_n\}$ and $\{y_n\}$. Then we have that 
...
Zero-padding was accomplished by adding $n$ zeros to each side of the sequence of $f$. Note that since $f$ is 3-dimensional, this means that symmetrically padding $f$ with $n$ zeros on each size results in a size increase of $8n^3$ in the amount of numbers that must be stored. In addition, to na{\"i}vely perform the calculations, we must also zero-pad the $A$ kernel. This results in a memory increase of $\mathcal{O}(64n^6)$. This of course then becomes restrictive as the number zeros increases since we will then run out of memory rather quickly. However, as the table below shows, even a modest increase in the number of zeros results in much lower error compared to the the straight-forward method presented earlier.
...
4 & 5.19E-07 & 1.14E-06 & 2.61E-07 & 2.93E-07 & 8.73E-08 & 9.58E-08\\
    \hline
  \end{tabular}
\caption{\label{tab:padding_m155}  formulations.}
\end{table}
...
4 & 2.13 & 0.64 & 11.40 & 4.19 & 51.07 & 13.63\\ 
    ...
    \hline
  \end{tabular}
\caption{\label{tab:timing_padding_m155}  }
\end{table}

As we can see from the table, the the fourier method still outperforms the direct method in time. However, the speed up drops dramatically as more zeros are added with the $N=21$ case having the speed up get halved by adding one zero. to the calculation, showing a severe cost for the dramatic increase in accuracy. The speed up drops by a third on $n_{\text{pad}} = 2$ to 20.97. 
